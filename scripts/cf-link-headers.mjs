 ...
 */
import { ENLACES } from './site-headers.mjs';
...
/* Se reescribe la fase ENTERA porque la API no tiene PATCH de una regla suelta
   dentro del entrypoint. Por eso se conservan las demás tal cual vinieron: si
   aquí se perdiera una, se caería la CSP del sitio sin hacer ruido. */
const otras = reglas.filter((r) => r.description !== DESCRIPCION);
const nueva = {
...
try {
  const res = await cf(`/zones/${zona.id}/rulesets/phases/${FASE}/entrypoint`, {
    method: 'PUT',
    body: JSON.stringify({ rules: [...otras, nueva] }),
  });
  console.log(`✔ regla ${mia ? 'actualizada' : 'creada'} · ${res.rules.length} reglas en la fase (${otras.length} intactas)`);
